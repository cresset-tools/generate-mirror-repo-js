...
async function transformMageOSMinimalProduct(composerConfig, instruction, metapackage, release) {
  ...
  const preservePackages = new Set([
    ...
    // laminas-i18n's View helpers extend Laminas\View\Helper\AbstractHelper, which
    // di:compile's class scanner loads — so without laminas-view the minimal edition
    // fatals at setup:di:compile. Keep it. (Boot-verified; report upstream too.)
...
/**
 * A LEANER minimal edition for the fully-modular (modulargento) distribution.
 * modulargento's decoupling breaks the couplings that force these storefront
 * features to stay in stock Mage-OS, so we can drop them from the minimal keep
 * set — every one below was empirically proven removable by booting the maker's
 * mageos-lite profile on modulargento (setup:install + di:compile + homepage).
 * Chains off the standard transform, then removes the extras.
 *
 * @param {{}} composerConfig
 * @param {repositoryBuildDefinition} instruction
 * @param {metapackageDefinition} metapackage
